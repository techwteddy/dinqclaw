...
const SCROLLBAR_CLASSES =
  "[&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-[3px] [&::-webkit-scrollbar-thumb]:bg-[#444444] hover:[&::-webkit-scrollbar-thumb]:bg-[#555555]";
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "default", size = "default", ...props }, ref) => {
    const variantClasses = {
      default: "bg-[#E8A045] hover:bg-[#E8A045]/80 text-[#010812]",
...
const VoiceRecorder: React.FC<VoiceRecorderProps> = ({
  ...
  React.useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- `time` is read only when recording stops; including it would restart the timer every tick
  }, [isRecording]);
        ...
        >
          ...
          <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
              ...
              <div className="flex items-center">
                ...
                <button
                  ...
                  className={cn(
                    "flex h-8 items-center gap-1 rounded-full border px-2 py-1 transition-all",
                    showCanvas
                      ? "border-[#E8A045] bg-[#E8A045]/15 text-[#E8A045]"
                      : "border-transparent bg-transparent text-[#9CA3AF] hover:text-[#D1D5DB]",
                  )}
                >
                  <div className="flex h-5 w-5 flex-shrink-0 items-center justify-center">
                    ...
                    >
                      <FolderCode
                        className={cn(
                          "h-4 w-4",
                          showCanvas ? "text-[#E8A045]" : "text-inherit",
                        )}
                      />
                    </motion.div>
                  </div>
                  <AnimatePresence>
                    {showCanvas && (
                      <motion.span
                        ...
                        exit={{ width: 0, opacity: 0 }}
                        transition={{ duration: 0.2 }}
                        className="flex-shrink-0 overflow-hidden text-xs whitespace-nowrap text-[#E8A045]"
                      >
                        Canvas
                      </motion.span>
                    )}
            ...
            >
              <Button
                ...
                className={cn(
                  ...
                  isRecording
                    ...
                    : isLoading
                      ? "bg-[#E8A045] text-[#010812] hover:bg-[#E8A045]/80"
                      : hasContent
                        ? "bg-[#E8A045] text-[#010812] hover:bg-[#E8A045]/80"
              ...
              >
                {isLoading ? (
                  <Square className="h-4 w-4 animate-pulse fill-[#010812]" />
                ) : isRecording ? (
                  <StopCircle className="h-5 w-5 text-red-500" />
                ) : hasContent ? (
                  <ArrowUp className="h-4 w-4 text-[#010812]" />
                ) : (
